...
public class UIMenu {
	...
	public void draw(int mouseX, int mouseY) {
		...
		if(this.x == (float)(-this.width)) {
			RenderUtil.drawImage(new ResourceLocation("Kyra/slidermenu_icon.png"), (int)this.x + this.width + 5, res.getScaledHeight() / 2 - 3, 6, 6);
		}
		float newX = (float)(this.isHovering(mouseX, mouseY)?0:-this.width);
		this.x = (float)RenderUtil.INSTANCE.getAnimationState((double)this.x, (double)newX, (double)(this.isHovering(mouseX, mouseY) ? (100.0F - this.x + 1.0F) * 10.0F : (Math.abs(this.x) + 1.0F) * 30.0F));
		Gui.drawCircle((int)this.x + this.width / 2 + 1, 36, 15, new Color(255, 255, 255));
		font.drawString(Client.CLIENT_name.substring(0, 1), this.x + (float)((this.width - font.getStringWidth(Client.CLIENT_name.substring(0, 1))) / 2) + 1, 16.0F, -1);
		byte tab_height = 20;
	    ...
	    for(Iterator var9 = this.slots.iterator(); var9.hasNext(); yStart += 21) {
	    	...
	    	slot.animationX = RenderUtil.INSTANCE.getAnimationState(slot.animationX, (double)(hover ? 1f : 0), 25.0D * slot.animationX + 1.0D);
	        if(hover) {
