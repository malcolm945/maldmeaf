...
import cn.Judgment.mod.Mod;
	...
	public class AutoL extends Mod {
		...
		public Value<Boolean> L = new Value("AutoL_L", true);
		public Value<Boolean> Wdr = new Value("AutoL_Wdr", true);
		public Value<Boolean> GG = new Value("AutoL_GG", false);
		public AutoL() {
	    ...
	    public void onPacket(EventChat e) {
	    	Random r = new Random();
			String fuck = intcihui[r.nextInt(800)];
			if (e.getMessage().contains("被" + mc.thePlayer.getName() + "击杀")
					...
					|| e.getMessage().contains(" 被击杀，击杀者: " + mc.thePlayer.getName())
					|| e.getMessage().contains(" was slain by " + mc.thePlayer.getName())
					|| e.getMessage().contains(" 被扔下虚空，击杀者： " + mc.thePlayer.getName())
					|| e.getMessage().contains("被" + mc.thePlayer.getName() + "扔下了悬崖。")) {
				if(L.getValueState().booleanValue()) {
				mc.thePlayer.sendChatMessage("/ac [Judgment]  " + e.getMessage().split("被")[0] + " " +" L "+ fuck);
				}
				if(Wdr.getValueState().booleanValue()) {
					mc.thePlayer.sendChatMessage("-wdr "+ e.getMessage().split("被")[0]);
				}
			}
			
			if(e.getMessage().contains(" 击杀数第一名 - ")) {
				if(GG.getValueState().booleanValue()) {
					mc.thePlayer.sendChatMessage("/ac  GG ");
			}
			}
				
	    
		if(e.getMessage().contains("奖励总览")) {
			if(GG.getValueState().booleanValue()) {
				mc.thePlayer.sendChatMessage("/ac [Judgment] 脑残你妈死了");
		}
		}
